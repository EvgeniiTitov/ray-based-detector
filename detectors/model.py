...
class DetectionModel(Model):
    ...
    CONFIG = os.path.join(
        os.getcwd(), "detectors", "dependencies", "{}.cfg"
    )
    ...
    def __init__(
        ...
        if not 0. <= conf <= 1. or not 0. <= nms <= 1.:
            raise Exception("Incorrect threshold(s) provided."
                            "Expected: (0, 1)")
    ...
    def _get_output_layers(self) -> t.List[str]:
        ...
        return [layers[i[0] - 1] for i in self._model.getUnconnectedOutLayers()]

    def predict(self, image: np.ndarray):
        blob = cv2.dnn.blobFromImage(
            image, 1 / 255.0, (self._image_size, self._image_size),
    ...
    for item in os.listdir("source"):
        ...
        if len(detections):
            for detection in detections:
                ...
                cv2.rectangle(image, (left, top), (right, bot), (0, 255, 0),
                              3, cv2.FONT_HERSHEY_PLAIN)
                cv2.putText(image, f"{cls}_{conf: .4f}", (left, top + 15),
