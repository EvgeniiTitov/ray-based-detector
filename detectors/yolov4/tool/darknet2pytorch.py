...
class MaxPoolDark(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(
                F.pad(
                    x, [padding3, padding4, padding1, padding2],
                    mode='replicate'
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                ...
                if len(layers) == 1:
                    if 'groups' not in block.keys() or\
                                                int(block['groups']) == 1:
                        x = outputs[layers[0]]
                        outputs[ind] = x
                    else:
                        ...
                        x = outputs[layers[0]][:, b // groups * group_id:
                                                  b // groups * (group_id + 1)]
                        outputs[ind] = x
                elif len(layers) == 2:
                    x1 = outputs[layers[0]]
        ...
        for block in blocks:
            if block['type'] == 'net':
                ...
                if batch_normalize:
                    ...
                    model.add_module(
                        'bn{0}'.format(conv_id), nn.BatchNorm2d(filters)
                    )
                ...
                if activation == 'leaky':
                    model.add_module(
                        'leaky{0}'.format(conv_id),
                        nn.LeakyReLU(0.1, inplace=True)
                    )
                elif activation == 'relu':
                    model.add_module(
                        'relu{0}'.format(conv_id), nn.ReLU(inplace=True)
                    )
